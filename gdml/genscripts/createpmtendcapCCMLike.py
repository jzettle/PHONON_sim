...
#All units in cm
total_endcap = 107.823 #full 2.5 m radius
fiducial_rad = 105.283 #2.5 m - 25 cm standoff
pmt_rad = 4.0*2.54 #8" PMT spaced at least 1/2" apart
pmtdiameter = 8.0*2.54
pmtplussd = (8.0*2.54 + 1.15*2.54)
pmt_height = 71.35505 #leave 30 cm from ends to allow for PMT placement in reality, R5912 PMTs are max 275 cm (leaves room for cables, etc. at either end
tpb_height = 71.1962 - (pmt_rad+0.00012)
...
pos_out = open('../simple_target_detector/pmt_endcap_positionsCCM.xml', 'w')
place_out = open('../simple_target_detector/pmt_placementCCM.xml', 'w')
pos_tpb_out = open('../simple_target_detector/tpb_endcap_positionsCCM.xml', 'w')
rot_tpb_out = open('../simple_target_detector/tpb_endcap_rotationsCCM.xml', 'w')
tpb_out = open('../simple_target_detector/tpb_placementCCM.xml', 'w')
endcap_out = open('../simple_target_detector/endcap_subsolidCCM.xml', 'w')
endcap_tpb_out = open('../simple_target_detector/endcap_tpb_subsolidCCM.xml', 'w')
hole_out = open('../simple_target_detector/endcap_pmthole_positionsCCM.xml', 'w')
...
for i,pmt in enumerate(pmt_circles):
    ...
    print("<rotation name=\"TPB_rot%d\" unit=\"deg\" x=\"%.3f\" y=\"%.5f\" z=\"%.3f\" />"%(i,0.0,0.0,180.0), file = rot_tpb_out)
    print("<rotation name=\"TPB_rot%d\" unit=\"deg\" x=\"%.3f\" y=\"%.5f\" z=\"%.3f\" />"%(i+len(pmt_circles),0.0,0.0,180.0), file = rot_tpb_out)

    print("<position name=\"TPB_pos%d\" unit=\"cm\" x=\"%.3f\" y=\"%.3f\" z=\"%.3f\" />"%(i, pmt.GetX1(), pmt.GetY1(), pmt_height), file=pos_tpb_out)
    print("<position name=\"TPB_pos%d\" unit=\"cm\" x=\"%.3f\" y=\"%.3f\" z=\"%.3f\" />"%(i+len(pmt_circles), pmt.GetX1(), pmt.GetY1(), -pmt_height), file=pos_tpb_out)
